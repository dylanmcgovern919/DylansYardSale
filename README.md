...
## Rubric requirement coverage (bulleted)

- ✅ **Database design with at least 3 entities / 3 tables**
  - `Product` → `products`
  - `Tag` → `tags`
  - `Order` → `orders`
  - `OrderItem` → `order_items`
  - many-to-many join table: `product_tags`

- ✅ **Contains CRUD operations**
  - Product CRUD in `ProductController`
  - Tag CRUD in `TagController`
  - Order CRUD in `OrderController`
  - Nested OrderItem CRUD in `OrderItemController`

- ✅ **Each entity has at least one CRUD operation**
  - `Product`, `Tag`, `Order`, and `OrderItem` each have CRUD paths.

- ✅ **One or more entities has all 4 CRUD operations**
  - `Product`, `Tag`, `Order`, and `OrderItem` all implement full CRUD.

- ✅ **At least one one-to-many relationship**
  - `Order (1) -> (many) OrderItem`
  - implemented by `Order.items` and `OrderItem.order`
  - exposed with nested routes under `/api/orders/{orderId}/items`

- ✅ **At least one many-to-many relationship + CRUD on relationship**
...
- ✅ **REST API tested through Swagger/Postman/ARC/front-end**
  - Swagger UI: `/swagger-ui/index.html`
  - OpenAPI docs: `/v3/api-docs`
  - controller/service tests included under `src/test/java`
...
### 4) Open Swagger
- `http://localhost:8080/swagger-ui/index.html` (this is the configured Swagger path from `springdoc.swagger-ui.path` in `application.yml`)

### 5) Use the API in Swagger (step-by-step)
- Start the app with `./mvnw spring-boot:run`.
- Open the correct Swagger URL: `http://localhost:8080/swagger-ui/index.html`.
- Expand any endpoint group (`products`, `tags`, `orders`, `order-item` routes).
- Click **Try it out** on an endpoint.
- Fill in path/query/body fields (for POST/PUT, paste JSON payload).
- Click **Execute** to send the request.
- Review:
  - response code (200/201/204/400/404)
  - response body JSON
  - curl request generated by Swagger
- To verify schema/contract directly, open `http://localhost:8080/v3/api-docs`.
